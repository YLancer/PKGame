...
public class GamePlayBackScriptPdk : MonoBehaviour
{
	...
	private string[] pathDirString = new string[]{ "B", "R", "L" };
	private int myIndex;
			...
			for (int k = 12; k >= 0; k--) {
				for (int j = 0; j < 4; j++) {
					...
					if (tempPai [point] > 0) {
						...
						temp = createGameObjectAndReturn ("Prefab/pdk/HandCard_s", parentList [i], Vector3.one);
						if (pathDirString [i] == DirectionEnum.Left) {
							temp.transform.Rotate (0, 0, -90);
						} else if (pathDirString [i] == DirectionEnum.Right) {
							temp.transform.Rotate (0, 0, 90);
						}

						temp.GetComponent<pdkCardScript> ().setPoint (point);
