...
public class GamePlayBackScriptDN : MonoBehaviour
{
		...
		for (int i = 0; i < aa.playerItems.Count; i++) {
			...
			for (int j = 0; j < 54; j++) {
				if (tempPai [j] > 0) {
					...
					temp = createGameObjectAndReturn ("Prefab/dn/HandCard_s_DN", parentList [realIndex[i]], Vector3.one);
					temp.GetComponent<pdkCardScript> ().isSmall = true;
					temp.GetComponent<pdkCardScript> ().setPoint (j,3);
					cards.Add (temp);
				}
			}
