...
public class GamePlayBackScript : MonoBehaviour {
	...
	void Start () {
		...
		guiObj.sprite = Resources.Load("Cards/Small/s"+aa.roomvo.guiPai,typeof(Sprite)) as Sprite;
				...
				if (tempPai [a] > 0) {
					...
					for (int b = 0; b < tempPai [a]; b++) {
						...
						if (pathDirString [i] == DirectionEnum.Left || pathDirString [i] == DirectionEnum.Right) {
							temp.GetComponent<TopAndBottomCardScript> ().setLefAndRightPoint (a,aa.roomvo.guiPai);
						} else {
							temp.GetComponent<TopAndBottomCardScript> ().setPoint (a,aa.roomvo.guiPai);
						}
	...
	public void chiCard (int avaIndex, int[] cardPoint)//其他人吃牌
	{
		//playerItems [avaIndex].showChiEffect ();
		SoundCtrl.getInstance ().playSoundByAction ("chi", playerItems [avaIndex].getSex ());
		...
		if (tempCardList != null) {
			for (int i = 0; i < tempCardList.Count; i++) {//消除其他的人牌碰牌长度
				...
				if (cardPoint [num] == temp.GetComponent<TopAndBottomCardScript> ().getPoint ()) {
		...
		if (path != null) {
			for (int i = 0; i < 3; i++) { //实例化其他人碰牌
				...
				switch (avaIndex) {
				case 0:
					obj.GetComponent<TopAndBottomCardScript> ().setPoint (cardPoint [i]);
					tempvector3 = new Vector3 (-380 + pengGangLists [avaIndex].Count * 200 + i * 60, 0);
					obj.transform.parent = pengGangParentList [avaIndex];
					break;
				case 1:
					obj.GetComponent<TopAndBottomCardScript> ().setLefAndRightPoint (cardPoint [i]);
	...
	private void frontChiCard (int avaIndex, int[] cardPoint)
	{
		...
		for (int i = 1; i < 3; i++) {
			...
			if (avaIndex == 1 || avaIndex == 3) {
				tempObj.GetComponent<TopAndBottomCardScript> ().setLefAndRightPoint (cardPoint [i]);
			} else {
				tempObj.GetComponent<TopAndBottomCardScript> ().setPoint (cardPoint [i]);
			}
	...
	private void pickCard(int avaIndex,int cardPoint){
		...
		case 0:
			...
			tempObj.GetComponent<TopAndBottomCardScript> ().setPoint (cardPoint,aa.roomvo.guiPai);
			break;
		case 1:
			tempObj = createGameObjectAndReturn ("Prefab/playBack/HandCard_R", parentList [avaIndex], new Vector3(0,250));
			tempObj.GetComponent<TopAndBottomCardScript> ().setLefAndRightPoint (cardPoint,aa.roomvo.guiPai);
			break;
		case 2:
