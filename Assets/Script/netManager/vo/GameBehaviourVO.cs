...
namespace AssemblyCSharp
{
	public class GameBehaviourVO
	{
		...
		{
			string[] temp = cardIndex.Split (new char[1]{ ',' });
			int[] pai = new int[temp.Length];
		...
		public int[] getOutCard(){
			string[] cardString = cardIndex.Split(',');
			int[] card = new int[cardString.Length];
			for (int i = 0; i < cardString.Length; i++) {
				card [i] = int.Parse(cardString [i]);
			}
			return card;
		}
	}
